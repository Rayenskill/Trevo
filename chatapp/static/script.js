...
/*
    All the JavaScript from the script.js file is here.
*/
document.addEventListener('DOMContentLoaded', () => {
    // Select all chat links and the chat display area
    const chatLinks = document.querySelectorAll('.chat-link');
    const chatDisplay = document.getElementById('chat-display-content');

    // Attach a click event listener to each chat link
    chatLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();

            // Get the unique chat ID from the data attribute
            const chatId = link.dataset.chatId;

            // Get the user's name from the link's text content
            const userName = link.textContent;

            // Update the chat display content
            chatDisplay.innerHTML = `
                <h3 class="chat-display-title">Chat with ${userName}</h3>
                <p>Loading messages for ${chatId}...</p>
            `;
            chatDisplay.classList.remove('chat-display-placeholder');
        });
    });
});


let url = `ws://${window.location.host}/ws/chat/${roomGroupName}/` // creates a specific URL for a WS connection chat.
let urlKwargs = `/chat/${roomGroupName}`
